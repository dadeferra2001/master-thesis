"""Centralized single-agent PPO over the 4-light joint action."""
...
def train(
    ...
    model = FactorizedJointActorCritic(
        obs_dim=obs_dim,
    ...
    minibatch_size = batch_size // int(train_cfg["num_minibatches"])

    obs_buffer = torch.zeros((num_steps, obs_dim), dtype=torch.float32, device=device)
    actions_buffer = torch.zeros((num_steps, num_agents), dtype=torch.long, device=device)
    logprobs_buffer = torch.zeros((num_steps,), dtype=torch.float32, device=device)
    rewards_buffer = torch.zeros((num_steps,), dtype=torch.float32, device=device)
    dones_buffer = torch.zeros((num_steps,), dtype=torch.float32, device=device)
    values_buffer = torch.zeros((num_steps,), dtype=torch.float32, device=device)
    ...
    for update in range(1, num_updates + 1):
        ...
        for step in range(num_steps):
            ...
            next_obs_np, reward, _, truncated, _ = env.step(action.squeeze(0).cpu().numpy())
            rewards_buffer[step] = float(reward)
            episode_return += float(reward)
            next_done = torch.as_tensor(float(truncated), dtype=torch.float32, device=device)
            ...
            if truncated:
                ...
                env.close()
                route_spec = sample_route(route_specs, rng)
                env = CentralizedGridEnv(config, route_spec["path"], seed=int(rng.integers(1_000_000)))
                next_obs_np, _ = env.reset(seed=int(rng.integers(1_000_000)))
                episode_return = 0.0
        ...
        b_actions = actions_buffer.reshape((-1, num_agents))
...
def load_controller(checkpoint_path: str | Path, device: torch.device | None = None) -> CentralizedController:
    ...
    model.eval()
    return CentralizedController(model=model, device=device, agent_order=checkpoint["agent_order"])
