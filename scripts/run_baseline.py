...
def main() -> None:
    ...
    for intensity in intensities:
        ...
        for route_spec in route_specs_for_split(manifest, split=args.split, intensity=intensity):
            run_dir = eval_dir("baseline", args.split, intensity, int(route_spec["seed"]), variant=variant)
        ...
        aggregate.update(
            {"algorithm": "baseline", "split": args.split, "intensity": intensity, "variant": variant or "vehicle"}
        )
        output_root = ROOT / "results" / "eval" / "baseline"
        if variant:
            output_root /= variant
